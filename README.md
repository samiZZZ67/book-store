...
4. In Telegram Notifications, add up to two admin usernames without `@`.
5. Each Telegram admin must send `/start` to the bot.
6. For local testing without a public webhook, run:

```powershell
python manage.py sync_telegram_admins
```

7. On Render, set the webhook after the service is live:
